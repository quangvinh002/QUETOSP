...
export default function Inventory({}) {
    ...
    const [getPackCodeList] = useLazyQuery(PACK_CODE_PAGING)
    const [createPackCode] = useMutation(CREATE_PACK_CODE)
    const [updatePackCode] = useMutation(UPDATE_PACK_CODE)
    ...
    useEffect(() => {
        ...
        getPackCodeList({
            ...
            .then((res) => {
                const { data, total } = res?.data?.PackCodeList
                setData(data)
                setTotalData(
                    res?.data?.PackStoreList?.data?.map((i) => {
                        return {
                            ...i,
                            display_name: i?.branch?.display_name
                        }
                    })
                )
                setTotal(total)
    ...
    useEffect(() => {
        if (filter?.from_date && filter?.to_date) {
            ...
            Axios.post('pack-request/sum-pack-request', {
                ...
                .then((res) => {
                    setSumData(
                        res?.data?.data?.filter((i) => i?.display_name !== null)
                    )
                })
                .finally(() => setLoading(false))
        } else {
    ...
    const sendPack = (values) => {
        ...
        updatePackCode({
            ...
            .then((res) => {
                if (res?.data?.updatePackCode?.pack_code) {
                    message.open({
    ...
    const newPack = (values) => {
        ...
        createPackCode({
            ...
            .then((res) => {
                if (res?.data?.createPackCode?.pack_code) {
    ...
    // const uniqueTotal = [
    //     ...new Set(totalData.map((item) => item?.branch?.display_name))
    // ]
                ...
                {/* <Col span={24}>
                    <Table
                        ...
                        <Column
                            title="6C90N"
                            render={(text, record) => {
                                const sum = totalData?.find(
                                    (i) =>
                                        i?.branch?.display_name === record &&
                                        i?.pack_code === '6C90N'
                                )?.amount
